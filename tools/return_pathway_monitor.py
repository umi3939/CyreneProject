...
3本の帰還経路（記憶想起→感情帯域変動、ポリシー選択→感情帯域変動、
他者仮説→感情帯域変動）が実運用中に発火した事実と、複数経路が同一ティック内で
同時に適用された際の合算帯域を、事後的に記録する仕組みである。

設計書: design_return_pathway_verification.md
...
_ALL_PATHWAYS = frozenset({PATHWAY_A, PATHWAY_B, PATHWAY_C})
...
class ReturnPathwayMonitor:
    ...
    def __init__(self, enabled: Optional[bool] = None) -> None:
        ...
        # 3経路同時発火の累積回数
        self._concurrent_3_count: int = 0
    ...
    def concurrent_3_count(self) -> int:
        """3経路同時発火の累積回数。"""
        return self._concurrent_3_count
    ...
    def record_firing(
        ...
        Args:
            pathway_id: 発火した帰還経路の識別子
                (PATHWAY_A, PATHWAY_B, PATHWAY_C のいずれか)
            tick_number: 発火が発生したティック番号
            emotion_deltas: 適用された帯域変動の各次元の値(変動量そのもの)
        """
        ...
        try:
            ...
            # ティック内バッファに追加(最大3件: 経路A/B/Cの各1件まで)
            # 同一経路の重複記録を防止
            ...
            record = {
                ...
                "emotion_deltas": dict(emotion_deltas) if emotion_deltas else {},
            }
            ...
            for record in self._tick_buffer:
                for dim, delta in record.get("emotion_deltas", {}).items():
                    if isinstance(delta, (int, float)):
                        combined_deltas[dim] = combined_deltas.get(dim, 0.0) + delta

            # 同時発火カウンタの更新
            if fire_count >= 2:
                self._concurrent_2plus_count += 1
            if fire_count >= 3:
                self._concurrent_3_count += 1
    ...
    def emit_session_summary(self) -> Optional[dict[str, Any]]:
        ...
        try:
            summary: dict[str, Any] = {
                ...
                "concurrent_3_count": self._concurrent_3_count,
            }
