...
の直近系列を保持し、その推移を4つの断面で段階的に記述する構造を提供する。

設計原則 (design_perceptual_context.md 準拠):
...
2. 4断面での知覚推移特徴量の記述 (4-section perceptual transition feature description)
...
class PerceptualContextState:
    ...
    # 断面別の推移特徴量スナップショット: 4断面それぞれの最新の特徴量
