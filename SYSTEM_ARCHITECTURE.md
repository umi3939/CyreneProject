...
| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 2 | 帰還方向偏り対策 — 合算帯域上限 | 条件付き推奨 | orchestrator_1tick_phases.py拡張 (596→824行): 3種類別独立合算上限(感情0.15/ドライブ0.10/ムード速度0.05)、全経路等価比例縮小。return_pathway_monitor.py拡張 (422→483行): 到達頻度記録。orchestrator.py +9行。47新テスト |
