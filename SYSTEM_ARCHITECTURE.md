...
| ディレクトリ | ファイル数 | 総行数 | 説明 |
|-------------|-----------|--------|------|
| psyche/ | 109 | 93,819 | 心理システム本体（orchestrator.py含む） |
| tests/ | 179 | 154,662 | 自動テストコード |
| src/ | 13 | 2,659 | 補助モジュール |
| tools/ | 15 | 13,419 | 検証・モニタリングツール群 |
| ルート | 7 | 3,933 | コアシステム |
| **合計** | **322** | **267,878** | |
...
| 7 | episodic_memory.py | 1,689 | 113 | 記憶 | エピソード記憶（自伝的記憶） |
| 8 | introspection_consumption.py | 1,455 | 94 | 内省 | 内省の消費層（読み取り可能断片の循環） |
| 9 | expectation_formation.py | 1,434 | 103 | 内省 | 予期・期待の形成（未来方向の連続性投射） |
| 10 | other_agent_model.py | 1,610 | 112 | 内省 | 他者モデル（他者状態の仮説的推測） |
| 10a | other_model_input_supply.py | 440 | 30 | 内省 | 他者モデル入力供給（external_context / reaction_log 生成） |
| 11 | emotional_memory_binding.py | 1,694 | 114 | 記憶 | 感情記憶の紐づけ（中長期感情痕跡） |
...
| 13d | text_dialogue_input.py | 1,548 | 102 | 入力 | テキスト対話入力経路（6段パイプライン・経路多様性・重複抑制・安全弁） |
| 13e | spontaneous_activation.py | 1,552 | 84 | 起動 | 自発起動経路（8断面交差・5段パイプライン・競合並立・安全弁） |
| 13f | value_orientation_validation.py | 1,228 | 88 | 検証 | 価値方向性実運用検証（8断面・6段パイプライン・差分並立・安全弁） |
| 13g | memory_forgetting_fixation.py | 1,128 | 85 | 記憶 | 記憶の忘却と固定化（8断面・6段パイプライン・段階忘却・復帰経路・安全弁） |
| 13h | action_result_observation.py | 1,638 | 128 | 観測 | 行動-結果の観測と蓄積（8断面・6段パイプライン・非正誤判定・時系列隣接記録・入力経路ラベル併記・安全弁4種） |
| 13i | other_model_dialogue_learning.py | 1,651 | 135 | 内省 | 他者観測の長期蓄積と仮説補助（8断面・8段パイプライン・相手別分離・仮説再生成方式・安全弁4種） |
...
| 13n | multi_path_recall.py | 1,282 | 178 | 記憶 | 記憶の多経路想起（3経路想起・感情連想/文脈連想(TF-IDF+cosine)/時間近接・経路等価性・顕著性バイアス抑制・ルーミネーション防止・忘却分離・TF-IDFインデックス動的構築・安全弁8種） |
...
| 13p | perceptual_context.py | 766 | 116 | 知覚 | 知覚入力の内部文脈化（3段パイプライン・5断面段階値列挙型・感情変化頻度/意図変化頻度/話題重複度/感情価推移方向/知覚構造化度・テキスト比較禁止・4経路遮断・安全弁7種） |
...
| 13t | persistent_commitment.py | 1,083 | テスト内 | 目標 | 持続的取り組み保持（transient_goal昇格が唯一生成経路・複数並行保持・強度依存非線形減衰・慣性時間減衰・4解除条件・認知記録FIFO・資源競合・バイアス上限<VO・安全弁6種・自己強化ループ4重遮断） |
...
| 13ac2 | expectation_perception_matching.py | 492 | 59 | 予期 | 予期-知覚照合記述（3断面構造的比較・5段階値・FIFO50件・正誤判定禁止・感情直接接続禁止・パターン抽出禁止・安全弁5種）Phase 26d2、enrichment #47 |
...
| 13ak | hypothesis_observation_pairing.py | 1,021 | テスト内 | 他者認知 | 仮説-観測の隣接対構成（6段パイプライン・時間的隣接のみ・正誤判定禁止・確認バイアス排除・相手別分離・ルーミネーション防止・安全弁7種） |
| 13al | other_hypothesis_emotion_return.py | 758 | 65 | 感情 | 他者仮説由来の感情帯域追加（4段パイプライン・キーワード辞書照合・帯域±0.02以下・memory_emotion_returnとの合算上限・ルーミネーション減衰・enrichment非露出・安全弁7種） |
...
| 7 | repeated_tendency.py | 846 | 35 | 目的 | 反復傾向（習慣）形成 |
| 8 | transient_goal.py | 800 | 34 | 目的 | 一時的目的選択 |
...
| 16 | scoped_goal.py | 648 | 40 | 目的 | スコープ目的（1ターン） |
...
| 30 | responsibility_manager.py | 221 | - | 責任 | 責任マネージャー |
| 31 | reaction.py | 276 | - | 感情 | 反応処理コア（SD-1~5帰還。構造分割済み→reaction_drive_dynamics/reaction_mood_update） |
| 31a | reaction_drive_dynamics.py | 509 | - | 感情 | SD-3 9断面ドライブ動態（reaction.pyから分離） |
| 31b | reaction_mood_update.py | 249 | - | 感情 | SD-4ムード自律化（reaction.pyから分離） |
| 32 | expression.py | 238 | - | 出力 | 表現生成（brain.py接続済み, psyche enrichment付き, 3層構造prompt） |
| 33 | perception.py | 398 | 294 | 入力 | 知覚処理（brain.py接続済み, LLM enrichment有効, 知覚バイアス: mood.valence→emotion_valence微弱加算） |
...
| 41 | orchestrator.py | 4,596 | 63 | 統合 | 全モジュール統合管理（PsycheOrchestrator, 71システム, save/load v44(68項目永続化), enrichment(5セクション/49項目), select_policy_dict含む, 帰還合算上限呼出し） |
| 42 | orchestrator_5tick_phases.py | 1,781 | - | 統合 | 5ティック帯域Phase実行（Phase 21-26系、Phase 26-EXP動的クールダウン・帯域拡大・累積安全弁） |
| 43 | save_load_warmup.py | 1,072 | 54 | 基盤 | セッション復帰時ウォームアップ（整合性検証、セッション差分記述enrichment #49） |
| 44 | orchestrator_enrichment.py | 978 | - | 統合 | enrichment生成ロジック分離（49項目/5セクション生成） |
| 45 | scoring_fluctuation.py | 826 | テスト内 | 判断 | スコアリングの構造的揺らぎ（5段パイプライン・参照偏在→振幅変調追加・安全弁5種） |
| 46 | persistence_helpers.py | 588 | 72 | 基盤 | 永続化ヘルパー（マイグレーションチェーンv1→v44、共通ヘルパー） |
| 47 | orchestrator_1tick_phases.py | 824 | - | 統合 | 毎ティック帯域Phase実行（Phase 1-7、帰還合算帯域上限） |
| 48 | enrichment_compression.py | 622 | 74 | 基盤 | enrichment圧縮（3段パイプライン、空状態統一、鮮度注釈） |
| 49 | stabilization_description.py | 516 | 71 | 内省 | 安定化の構造的記述（2断面・3段パイプライン・enrichment直接露出遮断・安全弁5種） |
| 50 | phase_execution_engine.py | 448 | テスト内 | 基盤 | Phase実行エンジン（全4帯域完全宣言化、ハンドラ方式、プロファイラ統合） |
| 51 | coefficient_registry.py | 662 | 35 | 基盤 | 係数レジストリ（JSON外部係数ファイル読み込み、読取専用辞書、3段フォールバック、係数履歴蓄積） |
| 52 | memory_emotion_return.py | 824 | テスト内 | 感情 | 記憶想起→感情帰還（方向連続カウント、追従速度変調≤10%、save/load対応） |
| 53 | phase_declaration.py | 1,493 | 53 | 基盤 | Phase宣言的定義（83 Phase定義レコード(frozen)×6帯域、データ依存グラフ導出、永続化/enrichmentマッピング） |
| 54 | decay_rate_modulation.py | 227 | 45 | 感情 | 感情減衰速度の経験依存変調（emotional_backdrop_cognition蓄積→±5%帯域） |
| 55 | window_size_modulation.py | 295 | 52 | 記憶 | ウィンドウサイズの経験依存変調（forgetting_recall_balance想起頻度→±20%帯域） |
| 56 | firing_frequency_modulation.py | 357 | 50 | 基盤 | 帰還経路発火→帯域変調（累計発火→log変換→帯域拡大、絶対上限1.5x、4経路） |

### 2.3 コアシステムファイル

| ファイル | 行数 | 主要クラス/関数 | 説明 |
|---------|------|----------------|------|
| brain.py | 1,415 | CyreneBrain | 2-call思考生成（perception+expression, psyche enrichment, save/load, LLM parse_percept, think_text/think_spontaneous, PIL直接受渡し, fear_level公開, policy suggestions透明化, ContextEntry/DialogueContextManager, パイプライン計測点, self_action_perception通知経路） |
| loop_interval_controller.py | 742 | LoopIntervalController | メインループ適応間隔制御（3経路比率記録、telemetry、覚醒度→ループ間隔変調±10%） |
...
| ファイル | 行数 | 説明 |
|---------|------|------|
| api_error_resilience.py | 485 | APIエラー耐性（5段階分類、バックオフ、リトライ、統合エラーハンドリング） |
| llm_wrapper.py | 434 | LLM抽象化レイヤー（画像対応コール、EXPRESSION_SYSTEM_PROMPT含む） |
...
| ファイル | 行数 | 説明 |
|---------|------|------|
| execution_monitor.py | 2,135 | Phase計測、圧縮比追跡、API追跡、EnrichmentDistributionMonitor、EnrichmentEffectivenessMonitor |
| long_term_sim.py | 2,917 | 長期挙動シミュレータ（5シナリオ、統計、差分、帰還経路検証、自我発現計測4モード: 状態分岐度/軌跡統計特徴量/係数A/B比較/応答一貫性プローブ） |
...
| pipeline_measurement.py | 1,253 | パイプライン計測（end-to-endレイテンシ、知覚辞書カバレッジ率） |
| dashboard.py | 655 | 統合ダッシュボード（6種モニタリングツール統合CLI表示） |
...
| return_pathway_monitor.py | 557 | 帰還経路モニター（5帰還経路発火記録、合算帯域記述、合算上限到達頻度記録） |
| policy_distribution_tracking.py | 710 | ポリシー選択分布の経時的変化追跡（スナップショット蓄積、文脈別分布、推移記述） |
| enrichment_efficacy_evaluation.py | 563 | enrichment実効性の定量評価（3段処理: 構造化蓄積→精度限界記述→セッション累積記述、安全弁7種） |
| phase_profiler.py | 581 | Phase単位実行時間プロファイリング（コンテキストマネージャ方式、FIFOバッファ、セッション累積カウンタ、安全弁7種） |
| __init__.py | 0 | パッケージ初期化 |

### 2.6 Claude Code MCP 自己観測ツール (~/.claude/tools/)

キュレネのpsycheモジュール群をClaude Codeのセッションベースアーキテクチャに適応させたMCPツール群。
MCP サーバー: 6サーバー構成（memory-tools 17 + self-observation 12 + persistent-cron 11 + discord 7 + http-fetch 1 + playwright）= 48+ツール（70追跡対象）

#### 感情系ツール (既存)
| ツール名 | モジュール | 行数 | テスト数 | 説明 |
|---------|-----------|------|---------|------|
| emotion_get | emotion_state.py | - | - | 3軸感情状態の取得 |
| emotion_update | emotion_state.py | - | - | 3軸感情状態の直接更新 |
| emotion_react | emotion_state.py + emotion_dynamics.py | - | - | 刺激に対する感情反応（stability_valve抑制+long_term_dynamics記録統合） |
| emotion_return | emotion_dynamics.py | - | - | ピーク/リバウンド帰還処理 |
| emotion_history | emotion_dynamics.py | - | - | 感情変化履歴の取得 |
| emotion_restore | emotion_state.py | - | - | 感情状態の復元 |

#### 自己観測系ツール (新規 — パイプライン順)
| ツール名 | モジュール | 行数 | テスト数 | 説明 |
|---------|-----------|------|---------|------|
| self_observe | self_model.py | 386 | 37 | READ-ONLY自己状態スナップショット（4ファセット: 感情/変化/記憶/統合） |
| self_difference | temporal_self_difference.py | 461 | 48 | 時間的自己差分（FIFO 10スナップショット、5段階magnitude、6種nature） |
| continuity_strain | continuity_strain.py | 396 | 62 | 自己連続性負荷（差分の持続性追跡、4段階level、自然減衰機構） |
| self_image | self_image_integration.py | 464 | 71 | 暫定的自己像統合（5ファセット+矛盾検出、完全ステートレス） |
| identity_coherence | identity_coherence.py | 229 | 56 | シフト信号の重なり度合い（4源泉→4段階coherence_level） |
| stability_check | stability_valve.py | 248 | 43 | 安定化バルブ（3指標→抑制係数0.3-1.0、emotion_reactに自動統合） |
| tone_check | tone_modulation.py | 239 | 33 | 推奨応答トーン（5種トーン、感情+動力学から算出） |
| long_term_record | long_term_dynamics.py | 453 | 38 | 長期動態の受動的記録（emotion_reactに自動統合） |
| long_term_stats | long_term_dynamics.py | (同上) | (同上) | 長期動態の統計取得 |

**合計**: 2,876行 / 388テスト / 8設計書(design_*_mcp.md)

#### セッション管理ツール
| ツール名 | 説明 |
|---------|------|
| session_start | セッション開始（必須） |
| session_end | セッション終了（summary/completed/pending/decisions/issues/next_actions） |
| self_snapshot | 全自己観測を一括実行 |

#### 成長システムツール (C22で追加)
| ツール名 | 説明 |
|---------|------|
| validate_lesson | 教訓のconfidence/applied_count追跡 |
| record_success | 成功パターンの記録 |
| search_successes | 成功パターンの検索 |
| update_mastery | 能力プロファイル更新 |
| mastery_report | 能力レポート生成 |
| growth_dashboard | 成長ダッシュボード |
| growth_health | 成長バランス監視 |
| find_lessons | コンテキスト関連教訓検索 |
| detect_lesson_conflicts | 教訓間の矛盾検出 |
| record_trajectory | 成功軌跡記録 |
| find_trajectories | 軌跡検索 |
| golden_paths | ゴールデンパス抽出 |
| record_transfer | 転移記録 |
| transfer_report | 転移レポート |
| create_aar | After-Action Review作成 |
| search_aars | AAR検索 |
| aar_report | AARレポート |
...
| ツール名 | 説明 |
|---------|------|
| memory_record | エピソード記憶の記録 |
| memory_search | 記憶の検索 |
| memory_verify | 記憶の検証 |
| memory_status | 記憶システム状態 |
| stm_write | 短期記憶への書き込み |
| stm_read | 短期記憶の読み出し |
| stm_restore | 短期記憶の復元 |
| activation_surface | 活性化表面の取得 |

#### 設計原則
- **READ-ONLY**: 観測モジュールは感情状態を変更しない
- **抽象記述のみ**: 生の数値を外部に出さず、カテゴリ・抽象テキストのみ
- **評価なし**: 良い/悪い等の価値判断を含めない
- **FIFO制約**: 全永続化にサイズ上限あり（無限蓄積防止）
- **自然減衰**: ストレイン等は差分解消で自動低下
...
### 9.12 Cycle 7 Tier 2: Phase完全宣言化・長期安定性・帰還検証

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 2 | Phase実行エンジン毎ティック帯域拡大 | 条件付き推奨 | phase_execution_engine.py拡張: 毎ティック帯域ハンドラ方式。全4帯域(10ティック/3ティック/5ティック/1ティック)がエンジン管理下に完全移行。orchestrator.pyのPhase実行が100%宣言的に |
| 10 | 500ティック安定性テスト | 条件付き推奨 | test_500tick_stability.py (602行/26テスト): 68フィールド整合性、中間save/load、enrichment圧縮比推移。ポリシー多様性は統計記録のみ（pass/fail閾値なし）。@pytest.mark.slow |
| 3 | sim帰還経路対応と動態改善検証 | 条件付き推奨 | long_term_sim.py拡張: return_pathway_monitor活用、帰還経路発火記録、状態動態改善の定量検証。test_long_term_sim_return_pathway.py (490行) |

**設計書**: design_phase_engine_1tick.md / design_500tick_stability_test.md / design_sim_return_pathway_verification.md
**テスト追加**: ~70テスト (26 slow + 44 fast)

### 9.13 Cycle 7 Tier 3: 接続面検証・テスト改善・モニタリング完成

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 4 | 代弁品質のフィードバック構造化 | 条件付き推奨 | expression_quality_verification.py (533行/89テスト): psyche方針-Gemini発話の対記録。事実記録のみ、一致度評価なし。開発者向け外部ツール |
| 8 | テスト基盤構造的整理 | 条件付き推奨 | test_structure_improvement.py (673行): 結合テスト追加（クロスモジュールデータフロー、帰還経路連鎖検証）。既存テスト削除なし |
| 7 | enrichment 48項目有効性検証 | 条件付き推奨 | execution_monitor.py +299行: EnrichmentEffectivenessMonitor。空/安定項目検出、トークン消費量推定。Gemini参照推定は除外。enrichment項目の直接削除なし |
| 9 | パイプライン計測 | 条件付き推奨 | pipeline_measurement.py (383行): 知覚-判断-代弁のend-to-endレイテンシ記録。brain.pyへの最小計測点挿入。テスト941行 |

**設計書**: design_expression_quality_verification.md / design_test_structure_improvement.md / design_enrichment_effectiveness.md / design_pipeline_measurement.md
**テスト追加**: ~238テスト (89+48+12+89)

**Cycle 7全10候補完了** — 全候補が既存構造の検証・最適化・保守性改善。新psycheモジュールゼロ。

### 9.14 Cycle 8 Tier 1: enrichment分離・エラーテスト

Cycle 8は全10候補が既存構造の検証・最適化・実運用品質向上（新psycheモジュールゼロ、Cycle 4以降一貫）。

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 1 | enrichment生成ロジックの構造的分離 | **推奨** | orchestrator_enrichment.py (939行): get_prompt_enrichment()の48項目/5セクション生成ロジックをorchestratorから物理的分離。orchestrator.py (4,767→4,392行)。ロジック変更ゼロ、Phase帯域分離と同一パターン |
| 5 | brain.pyエラー復帰品質テスト | **推奨** | test_brain_error_resilience.py (1,216行/59テスト): 5段階(知覚取得/構造化/内部更新/代弁/記憶保存) + 4段階(エラー分類/バックオフ/リトライ/統合) + 連鎖エラー。全モック、psyche変更なし |

**候補リスト**: candidates_cycle8.md (10候補)
**討論記録**: discussion_cycle8_20260302.md (2推奨/8条件付き推奨)
**設計書**: design_enrichment_separation.md / design_brain_error_test.md
**テスト追加**: 59テスト

### 9.15 Cycle 8 Tier 2: 動態検出・復帰検証・Phase依存可視化

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 4 | 動態停止検出と警告フレームワーク | 条件付き推奨 | tools/anomaly_detection.py (564行/63テスト): 4信号型(感情/駆動/帰還経路/enrichment変動)のゼロ変化率検出。FIFO蓄積→遷移時のみアラート記録。READ-ONLY、修復なし、save/load非対象 |
| 2 | セッション復帰時状態整合性検証 | 条件付き推奨 | save_load_warmup.py拡張 (230→926行/54テスト): ティック番号整合性、ウィンドウサイズ/FIFO上限検証、モジュール間時間的前提照合。警告のみ修復なし |
| 3 | Phase間データ依存関係の宣言的可視化 | 条件付き推奨 | tools/phase_dependency_validator.py (522行/78テスト): 未参照書込み検出、未生産読取り検出、同一帯域内順序不整合検出、帯域間暗黙前提可視化。CLI対応。Phase実行変更なし |

**設計書**: design_anomaly_detection.md / design_session_recovery_check.md / design_phase_dependency_declaration.md
**テスト追加**: 195テスト (63+54+78)

### 9.16 Cycle 8 Tier 3: 検証・計測・統合表示

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 8 | ポリシー選択長期効果検証 | 条件付き推奨 | tools/policy_selection_analysis.py (652行/81テスト): 15ポリシー選択分布分析、6断面寄与度記録、集中度計測。事実記述のみ、規範的判断なし |
| 7 | 知覚辞書網羅性検証 | 条件付き推奨 | tools/pipeline_measurement.py拡張 (383→640行): perception.pyヒューリスティック辞書カバレッジ率計測。辞書変更なし |
| 6 | 記憶保存パイプライン品質検証 | 条件付き推奨 | test_memory_pipeline.py (867行): summarize_and_save()パイプライン検証、memory_emotion_return帰還経路トリガー確認。全モック |
| 9 | 3経路ループ制御品質向上 | 条件付き推奨 | loop_interval_controller.py拡張 (421→509行) + test_loop_control_quality.py (927行): telemetry追加、3経路比率記録、適応間隔テスト拡充 |
| 10 | 統合ダッシュボード | 条件付き推奨 | tools/dashboard.py (628行) + test_dashboard.py (969行): 6種モニタリングツール(return_pathway_monitor/execution_monitor/pipeline_measurement/policy_selection_log/expression_quality_verification/anomaly_detection)の統合CLI表示。新データ収集なし |

**設計書**: design_policy_selection_analysis.md / design_perception_coverage.md / design_memory_pipeline_test.md / design_loop_control_test.md / design_dashboard.md
**テスト追加**: ~290テスト

**Cycle 8全10候補完了** — 全候補が既存構造の検証・最適化・実運用品質向上。新psycheモジュールゼロ（orchestrator_enrichment.pyはリファクタリング）。新tools 4(anomaly_detection/phase_dependency_validator/policy_selection_analysis/dashboard)。

### 9.17 Cycle 9: 「感じるが変わらない」問題への対処

Cycle 9は学習レイヤーレポート(cyrene_learning_layer_report.md)を起点として、「感じるが変わらない」問題（固定係数の問題）に取り組んだ。討論でNN/RL(L-3/L-4)は思想的矛盾により除外、既存構造の拡張で解決する方針を確定。全8候補（推奨1/条件付き推奨7、要再検討2は除外）。

#### Tier 1: SD-3断面追加（推奨）

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 6 | SD-3入力源拡張 | **推奨** | reaction.py拡張: DriveContextInputs 2フィールド追加、新断面関数2つ（行動多様性段階値→帯域0.02/軸、内部矛盾件数→帯域0.02/軸）、orchestrator_1tick_phases.py build_drive_context()追加。_TOTAL_CHANGE_LIMIT不変。52テスト |

#### Tier 2: 帰還経路追加・帯域拡大（条件付き推奨）

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 10 | Phase 26-EXP冷却期間動的化 | 条件付き推奨 | orchestrator_5tick_phases.py拡張: _derive_dynamic_cooldown()純粋関数、入力=max(arousal, drive_variation)、最短2ティック保証。84テスト |
| 4 | 行動結果偏り→ドライブ帯域帰還 | 条件付き推奨 | reaction.py拡張: 3段階値→等価平均→帯域≤0.03/軸。新断面_compute_result_diversity_return()純粋関数。36テスト |
| 8 | 感情帰還方向追跡→追従速度変調 | 条件付き推奨 | memory_emotion_return.py拡張: 方向連続カウント(減衰付き)、追従速度変調≤10%、増加方向のみ。reaction.py MoodContextInputs拡張。save/load対応。47テスト |
| 2 | Phase 26-EXP帯域拡大 | 条件付き推奨 | orchestrator_5tick_phases.py拡張: 対象A(reaction _TOTAL_CHANGE_LIMIT一時拡大、乗数上限1.3)、対象B(thought _SCORE_SECTION_BAND一時拡大)。非永続属性。34テスト |

#### Tier 3: 補助構造（条件付き推奨・低優先）

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 5 | 参照偏在度→揺らぎ振幅変調 | 条件付き推奨 | scoring_fluctuation.py拡張: 偏在度を第5入力源として変動度合成に追加。偏在度高→振幅微増、低→中立（微減禁止）。amplitude_cap不変。39テスト |
| 9 | セッション間差分記述 | 条件付き推奨 | save_load_warmup.py拡張: save時に全フィールド数値距離のスカラー要約を算出、load時にenrichment提供。段階値変換（事実記述のみ）。50テスト |
| 1 | 係数ファイル基盤 | 条件付き推奨 | coefficient_registry.py (新規): JSON外部係数ファイル読み込み、読取専用辞書、3段フォールバック。reaction.py/thought.py/value_orientation.py等6モジュール統合。data/coefficients.json。35テスト |

**候補リスト**: candidates_cycle9.md (10候補)
**討論記録**: discussion_learning_layer_20260304.md (学習レイヤー討論), discussion_cycle9_20260304.md (10候補討論)
**設計書**: design_sd3_input_extension.md / design_exp_cooldown_dynamic.md / design_result_diversity_return.md / design_emotion_return_tracking.md / design_exp_bandwidth_expansion.md / design_reference_fluctuation_link.md / design_session_difference.md / design_coefficient_file.md
**テスト追加**: 377テスト (52+84+36+47+34+39+50+35)

**Cycle 9全8候補完了** — 「感じるが変わらない」問題に対し、NN/RLを使わず既存構造の拡張で帰還経路を強化。SD-3断面5→9、Phase 26-EXP対象拡大(ドライブ上限+スコアリング帯域)、冷却動的化、感情帰還追跡速度変調、揺らぎ入力源追加、セッション差分認知、係数外部化。新psycheモジュール1(coefficient_registry)。

### 9.18 Cycle 10 Tier 1: 帰還経路5本検証・Cycle 9動態sim検証

Cycle 10はCycle 9の検証・安定化を中心に構成。Tier 1は推奨2候補（検証ツール拡張のみ、psyche変更なし）。

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 2 | 帰還経路5本統合検証 | **推奨** | return_pathway_monitor.py拡張 (329→421行): 経路D(ドライブ帯域帰還)/経路E(ムード追従速度帰還)追加。drive_deltas/mood_speed_deltas記録。5本同時発火合算(種類別独立)。同時発火カウンタ2+/3+/4+/5。orchestrator_1tick_phases.py (551→595行): 通知点2箇所追加。64新テスト |
| 3 | Cycle 9動態改善sim検証 | **推奨** | long_term_sim.py拡張 (998→1,258行): 3新シナリオ(c9_high_arousal/c9_monotone/c9_abrupt_shift)。cycle9_dynamicsレコード(4サブフィールド: drive_dynamics/exp_bandwidth/dynamic_cooldown/emotion_return_tracking)。統計・差分レポート拡張。26新テスト |

**候補リスト**: candidates_cycle10.md (10候補)
**討論記録**: discussion_cycle10_20260304.md (2推奨/7条件付き推奨/1要再検討)
**設計書**: design_return_pathway_5channel.md / design_cycle9_dynamics_verification.md
**テスト追加**: 90テスト (64+26)

### 9.19 Cycle 10 Tier 2: 係数レジストリ拡大・reaction.py分割

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 1 | coefficient_registry統合範囲拡大 | 条件付き推奨 | coefficient_registry.py拡張 (291→317行): memory_emotion_return(10定数)とother_hypothesis_emotion_return(8定数)の2名前空間追加。coefficients.json拡張(68→92行)。各モジュールの設定構造体をレジストリ読み込みに変更。等価変換のみ。43新テスト |
| 5 | reaction.py構造的分割 | 条件付き推奨 | reaction.py (978→275行): 感情処理コアのみ残留。reaction_drive_dynamics.py (509行): SD-3の8断面ドライブ動態。reaction_mood_update.py (249行): SD-4ムード自律化。全公開要素をreaction.pyから再公開（import互換性維持）。ロジック変更ゼロ |

**設計書**: design_coefficient_registry_expansion.md / design_reaction_split.md
**テスト追加**: 43テスト

### 9.20 Cycle 10 Tier 3: enrichment最適化・係数履歴・知覚多様性

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 7 | enrichment空項目スキップ | 条件付き推奨 | enrichment_compression.py拡張 (500→622行): 空/未蓄積項目のスキップ機能。項目別連続空カウンタ、再生成間隔上限。orchestrator_enrichment.py拡張 (939→978行)。項目等価性維持。55新テスト |
| 8 | 係数変動履歴記録 | 条件付き推奨 | coefficient_registry.py拡張 (317→496行): セッション間係数変動履歴のFIFO記録。外部ファイル。psyche非影響。63新テスト |
| 9 | 知覚入力多様性記録 | 条件付き推奨 | pipeline_measurement.py拡張 (640→926行): 6断面入力多様性記録。perception.py最小フック追加 (341→384行)。psyche非帰還。39新テスト |

**設計書**: design_enrichment_empty_skip.md / design_coefficient_history.md / design_perception_diversity.md
**テスト追加**: 157テスト (55+63+39)

**Cycle 10全9候補完了** — Tier 1(推奨2: 帰還経路5本検証+動態sim検証)、Tier 2(条件付き推奨2: 係数レジストリ拡大+reaction分割)、Tier 3(条件付き推奨3: enrichment最適化+係数履歴+知覚多様性)。バグ修正2件(既知テスト失敗)。候補4(NNなし係数微調整)はL-3/L-4と同じ構造的問題のため除外。候補6(Phase 26-EXP累積安全弁)と候補10(帰還方向偏り対策)はTier 1検証結果待ちのため次サイクルに繰り越し。新psycheモジュール2(reaction_drive_dynamics/reaction_mood_update — 分割のため)。

### 9.21 Cycle 11 Tier 1: Phase 26-EXP累積安全弁・reaction分割テスト

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 1 | Phase 26-EXP累積安全弁 | **推奨** | orchestrator_5tick_phases.py拡張 (1,598→1,781行): 同時発動検出→累積影響量算出→均等スケールダウン→連続発動減衰。coefficient_registry.py +6行、coefficients.json +5行。45新テスト |
| 8 | reaction分割テスト | **推奨** | test_reaction_split_boundary.py (1,003行/99テスト): 3層構成（再公開同一性25件+独立動作56件+呼出パス結合18件）。psycheロジック変更なし |

**候補リスト**: candidates_cycle11.md (10候補)
**討論記録**: discussion_cycle11_20260305.md (2推奨/8条件付き推奨)
**設計書**: design_exp_cumulative_safety.md / design_reaction_split_test.md
**テスト追加**: 144テスト (45+99)

### 9.22 Cycle 11 Tier 2: 帰還合算帯域上限・Phase依存再検証・orchestrator健全性テスト

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 2 | 帰還方向偏り対策 — 合算帯域上限 | 条件付き推奨 | orchestrator_1tick_phases.py拡張 (596→824行): 3種類別独立合算上限(感情0.15/ドライブ0.10/ムード速度0.05)。**事後討論(discussion_c11_safety_review_20260306.md)により比例縮小を無効化→監視のみに変更**。上限超過の検出・記録は維持、縮小関数は将来再有効化用に保持。return_pathway_monitor.py拡張 (422→483行): 到達頻度記録。orchestrator.py +9行。44テスト |
| 4 | Phase依存関係再検証 | 条件付き推奨 | phase_declaration.py拡張 (1,436→1,493行): Phase 21g/21h/26-EXP新設(80→83)、Phase 1 reads 7変数追加。phase_dependency_validator.py拡張 (522→530行): EXTERNAL_INPUT_VARIABLES 7エントリ追加。157新テスト |
| 5 | orchestrator構造健全性テスト | 条件付き推奨 | test_orchestrator_health.py (746行/18テスト): 3領域(Phase間データ受渡し7+enrichment生成パス6+4ファイル間依存5)。psycheロジック変更なし |

**設計書**: design_return_aggregate_cap.md / design_phase_dependency_update.md / design_orchestrator_health_test.md
**テスト追加**: 219テスト (44+157+18)

### 9.23 Cycle 11 Tier 3: enrichment実効性評価・帰還経路相互作用sim・係数レジストリ第2段階・save/load全フィールドテスト・enrichment消費量可視化

| # | 候補 | 討論判定 | 実装内容 |
|---|------|---------|----------|
| 3 | enrichment実効性体系評価 | 条件付き推奨 | tools/execution_monitor.py拡張 (1,663→2,135行): EnrichmentEffectivenessEvaluator。項目別安定性・空率・トークン消費推定・重複類似度。段階判定なし事実記述のみ。79新テスト |
| 6 | 帰還経路相互作用sim検証 | 条件付き推奨 | tools/long_term_sim.py拡張 (1,258→1,605行): 3新シナリオ(return_interaction_simultaneous/neutral/residual)。帰還経路同時発火・残留相互作用の定量記録。72新テスト |
| 7 | coefficient_registry第2段階 | 条件付き推奨 | psyche/coefficient_registry.py拡張 (502→518行): description_common名前空間(FIFO上限4種+ウィンドウサイズ3種+鮮度減衰1種)。記述層モジュール20+ファイルの設定構造体をレジストリ経由に等価変換。data/coefficients.json拡張(97→108行)。71新テスト |
| 9 | save/load全フィールドテスト | 条件付き推奨 | tests/test_save_load_field_coverage.py (901行/98テスト): v44全68フィールドの差分テスト自動化。ラウンドトリップ検証・型保持・欠損フィールド耐性。psycheロジック変更なし |
| 10 | enrichment消費量可視化 | 条件付き推奨 | tools/pipeline_measurement.py拡張 (926→1,253行): EnrichmentConsumptionMeasurement。セクション別文字数記録・セッション統計・FIFO履歴。psyche非帰還。71新テスト |

**設計書**: design_enrichment_effectiveness_eval.md / design_sim_return_interaction.md / design_coefficient_registry_stage2.md / design_save_load_field_test.md / design_enrichment_consumption_viz.md
**テスト追加**: 391テスト (79+72+71+98+71)

**Cycle 11全10候補完了** — Tier 1(推奨2: Phase 26-EXP累積安全弁+reaction分割テスト)、Tier 2(条件付き推奨3: 帰還合算帯域上限+Phase依存再検証+orchestrator健全性テスト)、Tier 3(条件付き推奨5: enrichment実効性評価+帰還経路相互作用sim+係数レジストリ第2段階+save/load全フィールドテスト+enrichment消費量可視化)。新psycheモジュールゼロ。全て既存構造の検証・最適化・安全弁強化。

### 9.24 包括的コードレビューとバグ修正

~130ファイル（約250,000行）を8グループに分けて包括的コードレビューを実施。検出された全HIGH問題と大部分のMED/LOW問題を修正。修正後に固定化リスク解析を実施し「低リスク」を確認。

**レビュー対象グループ:**
| グループ | ファイル数 | HIGH | MED | LOW |
|----------|-----------|------|-----|-----|
| G1: Foundation | 15 | 1 | 2 | 4 |
| G2: Emotion & Reaction | 14 | 0 | 4 | 5 |
| G3: Memory & Observation | 10 | 2 | 8 | 15 |
| G4: Introspection | 12 | 2 | 5 | 5 |
| G5: Goals, Decision, Output | 19 | 0 | 4 | 11 |
| G6: Perception, Input, Others | 14 | 0 | 12 | 18 |
| G7: Description, Responsibility, Motivation | 16 | 0 | 4 | 6+ |
| G8: Orchestrator + Tools + Src + Root | ~30 | 4 | 10 | 6 |
| **合計** | **~130** | **9** | **49** | **70+** |
...
| # | 問題 | 修正内容 |
|---|------|----------|
| G8-H4 | orchestrator.py save() 非アトミック書き込み | tmp + os.replace() パターンに変更 |
| G1-H1 | persistence_helpers.py load_field falsy値バグ | `if fdef.key not in data` + None/空dict ガード |
| G8-H1 | `'attempt' in dir()` 変数存在チェックバグ | llm_wrapper.py + api_error_resilience.py: attempt=0 初期化 |
| G4-H1 | introspection_trace.py 無制限リスト | deque(maxlen=200) に変更 |
| G8-M9 | orchestrator_5tick_phases.py .emotions.mood 不正アクセス | .mood.valence に修正（実バグ: AttributeError が静かにキャッチされていた） |

全件修正 (4 HIGH + 30 MED + 18 LOW, 40ファイル):
| カテゴリ | 修正件数 | 主要な修正 |
|----------|---------|-----------|
| HIGH | 4 | brain.py _memories公開化, memory_manager.py カウント修正, emotional_memory_binding.py index脆弱性, action_result_observation.py 到達不能コード |
| MED バグ/ロジック | 15 | from_dict time.time()→0.0 (6箇所), 不変性違反修正, 常にゼロ変数修正, MD5長拡大, 条件分岐修正 |
| MED パフォーマンス | 3 | brain.py文分割ヘルパー抽出, other_agent_model.py O(n^2)キャッシュ, memory_link.py regex境界 |
| MED コード品質 | 12 | coefficient_registry統合(reaction.py+responsibility.py), deprecated API更新, atomic write追加(persistence.py+state_manager.py), 共有定数化, vision.py basicConfig削除 |
| LOW | 18 | デッドコード削除, unused変数削除, import整理, ログ追加 |

**修正により改善されたコード品質指標:**
- アトミック書き込み: orchestrator.py + persistence.py + state_manager.py + responsibility_manager.py (4箇所)
- coefficient_registry統合: reaction.py (0.3定数) + responsibility.py (5定数) → data/coefficients.json
- from_dict安全性: 6モジュールでtime.time()→0.0に統一（永続化値の汚染防止）
- FIFO安全性: introspection_trace.py deque化（メモリリーク防止）

**固定化リスク解析結果:**
修正後に2グループに分けて固定化リスク解析を実施:
- GroupA (Memory/Core/Brain): **低リスク** — reaction.py係数外部化により可変性向上、全修正箇所で競合構造・時間減衰が維持
- GroupB (Perception/Emotion/Goals/Responsibility): **低リスク** — responsibility.py係数外部化により固定化リスク低減、新たな固定化パターン導入なし

**未修正（設計上妥当/低影響/大規模リファクタリング要）:**
- orchestrator.py fallback/engine間 ~500行重複 → 将来のリファクタリングサイクルで対応
- text_dialogue_input.py freshness decay 3重重複 → ヘルパー抽出で対応可能
- other_model_real_feed.py mutable/immutable混在 → APIリデザイン要
- context_sensitivity.py EMA alpha ティック間隔非対応 → 影響軽微
- パフォーマンス系 (O(n^2)探索等) → 現在のデータ規模では問題なし
- LOW 50+件 → 動作に影響なし（命名・ドキュメント・軽微なコードスタイル）

**レビューレポート**: code_review_consolidated_20260306.md + 8グループ別レポート
...
#### Tier 3 実装完了 (C13-5/6/3/8)

- **C13-5 覚醒度→ループ間隔変調**: loop_interval_controller.py +233行（509→742行）。覚醒度→ループ間隔±10%変調。単調変化検出安全弁+ヒステリシス復帰。psyche非接続。セッション境界消失。62テスト
- **C13-6 Phase単位実行時間プロファイリング**: tools/phase_profiler.py 新規581行。コンテキストマネージャ方式計測点。FIFOバッファ+セッション累積カウンタ。phase_execution_engine.py +82行（366→448行）統合。psyche構造的分離（TYPE_CHECKING import）。安全弁7種。76テスト
- **C13-3 enrichment実効性の定量評価**: tools/enrichment_efficacy_evaluation.py 新規563行。3段処理（構造化蓄積→精度限界記述→セッション累積記述）。項目別/セクション別/全体のFIFOバッファ。READ-ONLY。安全弁7種。76テスト
- **C13-8 知覚構造化度（第5断面）**: psyche/perceptual_context.py +107行（660→766行）。5段階StructuringDegree列挙型。3比率等分平均。既存4断面と同一構造パターン。67テスト

**設計書**: design_arousal_loop_modulation.md, design_phase_profiling.md, design_enrichment_efficacy_evaluation.md, design_perceptual_structuring_degree.md
**固定化リスク解析**: analysis_c13_5_arousal_loop.md / analysis_c13_6_phase_profiler.md / analysis_c13_3_enrichment_efficacy.md / analysis_c13_8_perceptual_structuring.md（全てLOW）
**コードレビュー**: code_review_c13_5_arousal_loop.md / code_review_c13_6_phase_profiler.md / code_review_c13_3_enrichment_efficacy.md / code_review_c13_8_perceptual_structuring.md（HIGH 0, MED 4, LOW 12）
**テスト合計**: 12,010テスト全通過 (+281)

**Cycle 13全10候補完了** — gap E-4「自我発現の定量的指標の不在」への構造的対応。新psycheモジュールゼロ。新tools 3(phase_profiler/enrichment_efficacy_evaluation/policy_distribution_tracking)。long_term_sim.py自我発現計測4モード化。anomaly_detection.py緩やかな収束検出追加。perceptual_context.py第5断面追加。loop_interval_controller.py覚醒度変調追加。~261,000行/12,010テスト

---

*このドキュメントはCyrene AI システムの完全な技術仕様書です。*
*総コード行数: ~261,000行 (+2,876行 MCP自己観測) / テスト数: 12,010 (+388 MCP自己観測)*
