...
| 32 | expression.py | 172 | - | 出力 | 表現生成（brain.py接続済み） |
| 33 | perception.py | 157 | - | 入力 | 知覚処理（brain.py接続済み） |
...
| 41 | orchestrator.py | 1,167 | 40 | 統合 | 全モジュール統合管理（PsycheOrchestrator, select_policy_dict含む） |

### 2.3 コアシステムファイル

| ファイル | 行数 | 主要クラス/関数 | 説明 |
|---------|------|----------------|------|
| brain.py | 608 | CyreneBrain | 2-call思考生成（perception+expression, PsycheOrchestrator委譲） |
...
brain.py との接続 (2-call構造):
  __init__()               → PsycheOrchestrator(memory_count=...)
  ...
  think_streaming() Phase7 → expression.render_expression(state, policy, ...)
  think_streaming() Phase2 → perception.parse_percept(screen_description)
